                    ...
                    if(roleNotAllow == Number(userRole)) {
                        ...
                        if(userRole == 1) {
                            ...
                            .then(function(resultSpo) {
                                console.log(resultSpo);
                                let adFold = ["car", "boat", "motorcycle"];
                                resultSpo.forEach(element => {
                                    adminComList.innerHTML += `
                                        <li class="adminComItem">
                                            <img src="/img/ads/${adFold[element.category_fk-1]}/${element.image}" class="adminSpoImg">
                                            <h2 class="adminSpoTit">
                                                ${element.company}
                                            </h2>
                                            <span class="adminSpoSpan">
                                                <button class="adminMessDeleteBtn" onclick="removeAd(${element.id});">
...
function removeAd(adId) {
    let conDelete = confirm("Er du sikker på at du vil slette denne sponsor? Handlingen kan ikke gøres om.");
    if (conDelete == true) {
        // ---------- post article ----------
        ...
        let request = new Request('/remove/sponsor', init);
        fetch(request)
        .then(response => {
            return response.json();
        })
        .then((resultSpoDel) => { 
            if(resultSpoDel.affectedRows == 1) {
                window.location = '/admin/sponsor';
            } else {
