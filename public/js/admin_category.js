                ...
                .then(function(resultUser) {
                    ...
                    if(roleNotAllow == Number(userRole)) {
                        ...
                        if(userRole == 1) {
                            ...
                            const adminCatCarSel = document.getElementById("adminCatCarSel");
                            const adminCatBoatSel = document.getElementById("adminCatBoatSel");
                            const adminCatBikeSel = document.getElementById("adminCatBikeSel");
                            ...
                            .then(function(resultNav) {
                                resultNav.forEach(element => {
                                    if(element.id == 2) {
                                        adminCatCarSel.selectedIndex = element.sorted-2;
                                    } else if(element.id == 3) {
                                        adminCatBoatSel.selectedIndex = element.sorted-2;
                                    } else if(element.id == 4) {
                                        adminCatBikeSel.selectedIndex = element.sorted-2;
                                    }
                                });
                            });

                            const editCatConfirm = document.getElementById("editCatConfirm");
                            editCatConfirm.addEventListener('click', (event) => {
                                let headers = new Headers();
                                headers.append('Content-Type', 'application/json');
                                let init = {
                                    method: 'PUT',
                                    headers: headers,
                                    body: JSON.stringify ({"car":adminCatCarSel.value, "boat":adminCatBoatSel.value, "bike":adminCatBikeSel.value}),
                                    cache: 'no-cache',
                                    mode: 'cors'
                                };
                                let request = new Request('/update/navigation', init);
                                fetch(request)
                                .then(response => {
                                    return response.json();
                                })
                                .then((resultNav) => { 
                                    if(resultNav.affectedRows == 1)  {
                                        location.reload();
                                    } else {
