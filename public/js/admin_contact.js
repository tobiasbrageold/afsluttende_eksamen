...
document.addEventListener("DOMContentLoaded", () => {
                ...
                .then(function(resultUser) {
                    ...
                    if(roleNotAllow == Number(userRole)) {
                        ...
                        if(userRole == 1) {
                            ...
                            .then(function(resultMess) {
                                resultMess.forEach(element => {
                                    ...
                                    adminComList.innerHTML += `
                                        <li class="adminComItem">
                                            ...
                                            <p class="adminComArtSub">
                                                Emne: ${element.subject}
                                            </p>
                                            <p class="adminComComm">
                                                ${element.message}
                                            </p>
                                            <p class="adminComArt">
                                                E-mailadresse: ${element.email}
                                            </p>
                                            <button class="adminMessDeleteBtn" onclick="removeMessage(${element.id});">
    ...
    const aboutStreet = document.getElementById("aboutStreet");
    const aboutZip = document.getElementById("aboutZip");
    const aboutCity = document.getElementById("aboutCity");
    const aboutCountry = document.getElementById("aboutCountry");
    const aboutPhone = document.getElementById("aboutPhone");
    const aboutFax = document.getElementById("aboutFax");
    ...
    .then(function(resultAbout) {
        aboutStreet.value = resultAbout[0].street;
        aboutZip.value = resultAbout[0].zip_code;
        aboutCity.value = resultAbout[0].city;
        aboutCountry.value = resultAbout[0].country;
        aboutPhone.value = resultAbout[0].phone;
        aboutFax.value = resultAbout[0].fax;
        aboutMail.value = resultAbout[0].email;
    });

    const aboutSubmit = document.getElementById("aboutSubmit");
    aboutSubmit.addEventListener('click', (event) => {
        if(aboutStreet.value < 2 || aboutZip.value < 2 || aboutCity.value < 2 || aboutCountry.value < 2 || aboutPhone.value < 2 || aboutFax.value < 2 || aboutMail.value < 2) {
            ...
            let init = {
                method: 'PUT',
                headers: headers,
                body: JSON.stringify ({"street":aboutStreet.value, "zip":aboutZip.value, "city":aboutCity.value, "country":aboutCountry.value, "phone":aboutPhone.value, "fax":aboutFax.value, "email":aboutMail.value}),
                cache: 'no-cache',
                mode: 'cors'
            };
            let request = new Request('/update/about', init);
            fetch(request)
            .then(response => {
                return response.json();
            })
            .then((resultUpAbout) => { 
                if(resultUpAbout.affectedRows == 1) {
                    location.reload();
                } else {
...
function removeMessage(messageId) {
    ...
    if (conDelete == true) {
        ...
        let request = new Request('/remove/message', init);
        fetch(request)
        .then(response => {
            return response.json();
        })
        .then((resultDelMess) => { 
            if(resultDelMess.affectedRows == 1) {
                location.reload();
            } else {
                alert('Der skete en fejl, prøv at genindlæs siden.');
            }
