...
document.addEventListener("DOMContentLoaded", () => {
    if(readCookie('usertoken') || readCookie('userid')) {
        ...
        signInSubmit.addEventListener('click', (event) => {
            ...
            if(signInUsername.value.length < 2) {
                alert('Dit brugernavn er for kort.');
            } else {
                if(signInPassword.value.length < 2) {
                    alert('Din adgangskode er for kort.');
                } else {
                    let userToken = createToken();
                    let init = {
                        method: 'POST',
                        headers: headers,
                        body: `{"username":"${signInUsername.value}", "password":"${hashMd5(signInPassword.value)}", "token":"${userToken}"}`,
                        cache: 'no-cache',
                        mode: 'cors'
                    };
                    let request = new Request('/admin/login', init);
                    fetch(request)
                    .then(response => {
                        return response.json();
                    })
                    .then((resultUser) => { 
                        if(resultUser.message == "noUser") {
                            alert('Ingen bruger er registreret med dette brugernavn.');
                        } else if(resultUser.message == "noPass") {
                            alert('Adgangskoden er forkert for denne bruger.');
                        } else if(resultUser.message == "success") {
                            // ---------- user logged in ----------
                            createCookie('usertoken', userToken, 30);
                            createCookie('userid', resultUser.userId, 30);
                            window.location = '/admin';
                        }
                    });       
                }
...
function createToken(tokenLength = 32) {
    ...
    for (let i = 0; i < tokenLength; i++) {
        token += possChar.charAt(Math.floor(Math.random()*possChar.length));
    }
    return token;
}
